...
Run the self-hosted OSS CLI as a pipeline step: a diff-scoped scan on every PR, fully in your infra, free (BYO LLM key), no external account. Requires Docker on the runner. Fails the build on validated findings and emits SARIF 2.1.0.
